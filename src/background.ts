...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  switch (command) {
    ...
    case 'tabid':
      chrome.tabs.query({ active: true, currentWindow: true }, tabs =>
        sendResponse({ result: 'success', tabId: tabs[0].id }),
      );
      break;
    case 'checktabid':
      break;
    case 'findElement':
      ...
      sendMessageToActiveTab({ action: 'FIND_ELEMENT', data: data, sendResponse });
      break;
          ...
          if (!response[0].result) {
            setTimeout(() => {
              ...
              sendResponse({
                success: false,
                error:
                  '클릭 후 페이지 변경이 감지되지 않았습니다. 클릭 이벤트가 없는 엘리먼트일 수 있습니다.',
              });
            }, 3000);
          }
        })
